...
class App extends Component {
  ...
  componentDidMount() {
    axios.get('http://18.212.23.167:3002/api/recommended')
      .then((results) => {
  ...
  updateLiked(elementId, liked) {
    return axios.put(`http://18.212.23.167:3002/api/recommended/${elementId}`, { liked });
  }
  ...
  viewSwitcher(event) {
    ...
    if (str === '1') {
      ...
      axios.get(`http://18.212.23.167:3002/api/recommended/hello/${str}`)
        .then((results) => {
          const topFour = results.data.slice(0, 4);
